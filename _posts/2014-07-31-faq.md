...
```shell
perl -MStorable -MDBI -MDBD::MariaDB -e 'print "OK\n";'
```
...
### I get "Can't locate DBD/MariaDB.pm in @INC (you may need to install the DBD::MariaDB module)"

Since MailWatch version 1.2.24 perl library `DBD::mysql` has been replaced by `DBD::MariaDB` to fully support UTF8 and MariaDB server.

You need to install `DBD::MariaDB`:
...
it is highly probable that your version of `DBD::mysql` is outdated and doesn't support full UTF-8 encoding of email subject, please follow [upgrading DBD::MariaDB instructions]({% post_url 2017-03-23-getting-started %}) to fix this error.
